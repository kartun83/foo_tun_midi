#!/usr/bin/env ruby
# Generate the Xcode project for foo_tun_midi (standalone, no monorepo deps).
...
# Search-path prefix: absolute SDK paths are used as-is; relative ones resolve
# against the project directory (the repo root, where the .xcodeproj is written).
SDK_SEARCH = SDK_PATH.start_with?('/') ? SDK_PATH : "$(PROJECT_DIR)/#{SDK_PATH}"

# FluidSynth (Homebrew keg). arm64-only on Apple Silicon.
FLUIDSYNTH_PREFIX = ENV['FLUIDSYNTH_PREFIX'] || `brew --prefix fluid-synth 2>/dev/null`.strip
FLUIDSYNTH_PREFIX = "/opt/homebrew/opt/fluid-synth" if FLUIDSYNTH_PREFIX.empty?

# Version from src/version.h (single source of truth).
def read_midi_version
  vfile = File.join(File.dirname(__FILE__), '..', 'src', 'version.h')
  File.readlines(vfile).each do |line|
    return $1 if line =~ /#define\s+MIDI_VERSION\s+"([^"]+)"/
  end
  abort "ERROR: MIDI_VERSION not found in src/version.h"
end
COMPONENT_VERSION = read_midi_version
# Build number: major*10 + minor + 1 (0.1.0 => 2, matching the suite convention).
...
# Add Prefix.pch and fb2k_sdk.h references
